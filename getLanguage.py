...
def callMallet(inputText, classifier):
    ...
    if blurb_mallet != "\n":
        ...
        open('D:\CodeMix\\tmp\\temp_testFile.txt', 'w').write(blurb_mallet)
        ef.main('D:\CodeMix\\tmp\\temp_testFile.txt')
        # initialize t7 to track time taken by mallet
        t7 = time.time()
        finalPath = "D:\CodeMix\mallet-2.0.8\\bin\mallet classify-file --input D:\CodeMix\\tmp\\temp_testFile.txt.features --output D:\CodeMix\\tmp\\temp_testFile.txt.out --classifier D:/CodeMix/classifiers/HiEn.classifier"
        #print(finalPath)
        # call mallet to get probability output
        os.system("D:\CodeMix\mallet-2.0.8\\bin\mallet classify-file --input D:\CodeMix\\tmp\\temp_testFile.txt.features --output D:\CodeMix\\tmp\\temp_testFile.txt.out --classifier D:/CodeMix/classifiers/HiEn.classifier")
...
def langIdentify(inputText, classifier):
    ...
    for line in inputText:
        ...
        realOut = [list(word.split("/")) for word in realOut]
        # generate output
